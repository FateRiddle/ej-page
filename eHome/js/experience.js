...
$(function() {
	...
	$('.sli_pic').eq(0).show().siblings('.sli_pic').hide();
	//调用showTime()函数（轮播函数）
	showTime();
...
function Show() {
	//在这里可以用其他jquery的动画
	$('.sli_pic').eq(i).fadeIn(500).siblings('.sli_pic').hide();

$('.nav_num1_ul_2').css('margin-top',-(i*120));
//	$('.nav_num1_ul_2 li').eq(i).addClass('add_li_show').siblings('li').removeClass('add_li_show');
...
$(function() {
	$(".nav_title ul li").click(function() {
		...
		$(".nav_content>div").eq($(this).index()).show().siblings("div").hide();
	});
})
...
$(document).ready(function() {
	$(".nav .nav_a a:eq(7)").css("color", "#FFC61E");
	$(".search").click(function(){
		$(".search_show").show(300);
	});
	$(".search_show_end").click(function(){
		$(".search_show").hide(300);
	})
});