 ...
 */
//首页banner轮播
var oLb = document.getElementsByClassName('lb');
var oUl=document.getElementById('smallCir');
var oli=oUl.getElementsByTagName("li");
var oImg=oLb[0].getElementsByTagName("img");
var n=0;
var timer=null;
$(window).resize(function(){
	var winW=window.innerWidth;
	for(var j=0;j<oImg.length;j++){
		oImg[j].style.width=winW+"px";
	}
	oLb[0].style.width=winW*4+"px";
	oLb[0].style.left = (-winW*n) + "px";
	clearInterval(timer);
	carsouelIndex();
});
function carsouelIndex(){
for(var i=0;i<oli.length;i++){
	oli[i].index=i;
oli[i].addEventListener("click",function(){
	clearInterval(timer);
	n=this.index;
	oLb[0].style.left = (-(oLb[0].offsetWidth / 4)*n) + "px";
	for(var j=0;j<oli.length;j++){
		oli[j].style.backgroundColor="#666";
	}
	oli[n].style.backgroundColor="#ffc519";
	timer=setInterval(function(){moveRightTop()},3000)
...
function moveRightTop(){
	...
	if(n>2){
		oli[0].style.backgroundColor="#ffc519";
	}else{
		oli[n].style.backgroundColor="#ffc519";
	}
}
...
//主材包
var oPk = document.getElementsByClassName('pattem_kind');
var oPs = document.getElementsByClassName('pattem_span1');
var oPi = document.getElementsByClassName('pattem_img');
var oAd = document.getElementsByClassName("Amp_div");

var subscript = 0;
for(var j = 0; j < oAd.length; j++) {
	oAd[j].index = j;
	oAd[j].onclick = function() {
		oAd[subscript].style.color = 'black';
		subscript = this.index;
		this.style.color = '#ffc519';
		oPi[0].style.backgroundImage = "url('../images/bg" + this.index + '-' + '0' + ".jpg')";
		oPs[0].innerHTML = '客厅'
	};
	for(var i = 0; i < oPk.length; i++) {
		oPk[i].index = i;
		oPk[i].onmouseover = function() {

			oPs[0].innerHTML = this.children[1].children[0].innerHTML
			oPi[0].style.backgroundImage = "url('../images/bg" + subscript + '-' + this.index + ".jpg')";
		}
	}
}

//家居
//var oFd = document.getElementsByClassName('fitment_div');
//var oFb = document.getElementsByClassName('fitment_bg');
//var index1 = 0;
//for(var i = 0; i < oFd.length; i++) {
//	oFd[i].index = i;
//	oFd[i].onclick = function() {
//		oFd[index1].style.backgroundColor = '#929090';
//		index1 = this.index;
//		this.style.backgroundColor = '#ffc519';
//		oFb[0].style.backgroundImage = "url('../images/fitment" + this.index + ".jpg')"
//	}
//}

//家电
//var oAp = document.getElementsByClassName('appliance_div');
//var oAb = document.getElementsByClassName('appliance_bg');
//var index2 = 0;
//for(var i = 0; i < oAp.length; i++) {
//	oAp[i].index = i;
//	oAp[i].onclick = function() {
//		oAp[index2].style.backgroundColor = '#929090';
//		index2 = this.index;
//		this.style.backgroundColor = '#ffc519';
//		oAb[0].style.backgroundImage = "url('../images/appliance" + this.index + ".jpg')"
//	}
//}
//设计师
var oSd = document.getElementsByClassName('stylist_div');
var oSp = document.getElementsByClassName('stylist_projectors');
var index = 0;
for(var i = 0; i < oSd.length; i++) {
	oSd[i].index = i;
	oSd[i].onclick = function() {
		oSd[index].style.color = 'black';
		oSp[index].style.display = 'none';
		index = this.index;
		oSp[index].style.display = 'block';
		this.style.color = '#ffc61e';
	}
}
//案例

var oCd = document.getElementsByClassName('case_div');
for(var i = 0; i < oCd.length; i++) {
	oCd[i].onmouseover = function() {
		this.children[2].style.display = 'none';
		this.children[3].style.display = 'none';
		this.children[5].style.display = 'block';
		this.children[6].style.display = 'block';
	}
	oCd[i].onmouseout = function() {
...
//隐藏二维码
$(function() {
	$(".index_Xf").hover(function() {
		$(".index_hide").show(500);
	});
	$(".index_Xf").mouseleave(function() {
		$(".index_hide").hide(500);
	});
});
...
$(function(){
	$(".now-button").click(function(){
		location.href="../html/null.html";
	});
	$(".index-div4").click(function(){
		location.href="../html/null.html";
	});
});
...
$(document).ready(function() {
	$(".nav .nav_a a:eq(0)").css("color", "#FFC61E");
